...
/* Library function (return bitfield) */
static int bitfield(lua_State* L) {
	...
	case LUA_TUSERDATA: /* Template bitfield */
		w = UDBITS((bf_ud*)luaL_checkudata(L, 1, METANAME)) + 1;
	...
	default: /* Initialiser list only - scan initialisers for total width */
		...
		for (i = lt; (i <= rt); i++) {
			...
			case LUA_TSTRING:
				...
				break;
			case LUA_TUSERDATA:
				w += UDBITS((bf_ud*)luaL_checkudata(L, i, METANAME)) + 1;
				break;
			default:
	...
	if (tud > 0) {setbitfield(ud, (bf_ud*)luaL_checkudata(L, tud, METANAME), 0);}
	x = 0; for (i = rt; (i >= lt); i--) { /* Process Initialisers */
		...
		case LUA_TNUMBER:
			...
			break;
		case LUA_TUSERDATA:
			x = setbitfield(ud, (bf_ud*)luaL_checkudata(L, i, METANAME), x);
			break;
		case LUA_TSTRING:
...
static int bf_len(lua_State* L) {
	lua_pushunsigned(L, (UDBITS((bf_ud*)luaL_checkudata(L, 1, METANAME))) + 1);
	return 1;
}

/* Metamethod for __tostring */
static int bf_tostring(lua_State* L) {
	bf_ud* ud = (bf_ud*)luaL_checkudata(L, 1, METANAME);
...
static int bf_newindex(lua_State* L) {
	...
	case LUA_TUSERDATA: /* Must be a bitfield */
		if ((ty >= 0) && (ty != BF_TBITFIELD))
			return luaL_argerror(L, 3, "Bitfield required for this range");
		sd = (bf_ud*)luaL_checkudata(L, 3, METANAME);
		...
		break;
	case LUA_TNUMBER: /* Must be an unsigned value */
		if ((ty >= 0) && (ty != BF_TUNSIGNED))
			return luaL_argerror(L, 3, "Number required for this range");
	...
	default: /* Only other possibility is a boolean value */
		if (ty >= 0) if ((ty != BF_TBOOLEAN) || (ix != ep))
			return luaL_argerror(L, 3, "Boolean required for this range");
...
static int bf_concat(lua_State* L) {
	...
	if (lua_type(L, 1) != LUA_TBOOLEAN) ud1 = (bf_ud*)luaL_checkudata(L, 1, METANAME);
	if (lua_type(L, 2) != LUA_TBOOLEAN) ud2 = (bf_ud*)luaL_checkudata(L, 2, METANAME);
...
static int bf_eq(lua_State* L) {
	...
	ud1 = (bf_ud*)luaL_checkudata(L, 1, METANAME);
	ud2 = (bf_ud*)luaL_checkudata(L, 2, METANAME);
	lua_pushboolean(L, 0);
...
/* Functions for bitfield metatable */
static const luaL_Reg bf_meta[] = {
	{"__len", bf_len},
	{"__index", bf_index},
...
/* Functions for library table */
static const luaL_Reg lb_meta[] = {
	{"bitfield", bitfield},
	{"bitrange", bitrange},
	{NULL, NULL}
};
...
LUAMOD_API int luaopen_bitfield (lua_State *L) {
	...
	/* Store metatable for bitfield in registry table */
	luaL_newmetatable(L, METANAME);
	luaL_setfuncs(L, bf_meta, 0);
	lua_pushstring(L, "bitfield");
	lua_setfield(L, -2, "__type");
	lua_pop(L, 1);
	/* Return library table for bitfield library */
	luaL_newlib(L, lb_meta);
